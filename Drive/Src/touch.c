...
#include "flash.h"
#include "main.h"
//触摸屏驱动(支持ADS7843/7846/UH7843/7846/XPT2046/TSC2046/OTT2001A/GT9147/FT5206等) 
_m_tp_dev tp_dev=
{
	TP_Init,
	TP_Scan,
...
#define Flash_Save_Addr 	0x080FF800	
//保存校准参数										    
void TP_Save_Adjdata(void)
{  
	u8 temp;		
  STMFLASH_Write((u8*)&tp_dev.xfac,Flash_Save_Addr,14);	//强制保存&tp_dev.xfac地址开始的14个字节数据，即保存到tp_dev.touchtype
	temp=0x0A;
	STMFLASH_Write((u8*)&temp,Flash_Save_Addr+14,1);	//在最后，写0X0A标记校准过了
}
//得到保存在EEPROM里面的校准值
//返回值：1，成功获取数据
//        0，获取失败，要重新校准
...
{		  
  u8 temp;
  STMFLASH_Read((u8*)&temp,Flash_Save_Addr+14,1);	//读取标记字,看是否校准过！ 
	if(temp==0X0A)//触摸屏已经校准过了			   
 	{ 
		STMFLASH_Read((u8*)&tp_dev.xfac,Flash_Save_Addr,14);//读取之前保存的校准数据 
...
{		
	...
	GPIO_InitStruct.Pin = GPIO_PIN_5|GPIO_PIN_11|GPIO_PIN_13;	   
  ...
  GPIO_InitStruct.Pull = GPIO_PULLUP;
	...
	TP_Read_XY(&tp_dev.x[0],&tp_dev.y[0]);//第一次读取初始化	 
	 if (Flash_Init()!= HAL_OK)//初始化flash,如果未初始化成功，则进入错误处理
    {
      Error_Handler();
    }
